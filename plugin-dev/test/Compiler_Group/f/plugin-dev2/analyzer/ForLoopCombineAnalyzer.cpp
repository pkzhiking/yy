 ...
 */
#include "include/ForLoopCombineAnalyzer.h"
	...
	if ( NodeProcessor::isForStmt( preStmt ) )
		if ( checkTwoLoop( node, preStmt, context ) ){
			...
			stringstream reportMsgStream;
							reportMsgStream<<"ForLoopCombineAnalyzer: This for loop may be combine with previous for loop "<<endl;
							string reportMsg = reportMsgStream.str();
							ReportManager::getInstance().insertReport(SrcManager::getInstance().getFullFileName(),Util::stringToInt( NodeProcessor::getFieldStr( node, "line" ) ),reportMsg);
							ProblemList::GetInstance().Add((char*)"ForLoopCombineAnalyzer", SrcManager::getInstance().getFullFileName(), ContextProcessor::inferSrcPositon(context));
