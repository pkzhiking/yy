	...
	(function(window, undefined) {
		...
		Expr = Sizzle.selectors = {
			...
			pseudos : {
				...
				// Positional types
				"first" : createPositionalPseudo(function(matchIndexes, length,
						argument) {
					return [ 0 ];
				}),
			...
			while (soFar) {
				...
				for (type in Expr.filter) {
					if ((match = matchExpr[type].exec(soFar))
							&& (!preFilters[type] ||
							// The last two arguments here are (context, xml)
							// for backCompat
		...
		function setMatcher(preFilter, selector, matcher, postFilter,
			...
			return markFunction(function(seed, results, context, xml) {
				// Positional selectors apply to seed elements, so it is invalid
				// to follow them with relative ones
				if (seed && postFinder) {
					return;
				}

				var i, elem, postFilterIn, preMap = [], postMap = [], preexisting = results.length,
		...
		if (document.querySelectorAll) {
			(function() {
				...
				rbuggyMatches = [ ":active", ":focus" ], matches = docElem.matchesSelector
			...
			.extend({
				...
				ajax : function(url, options) {
					...
					if (s.crossDomain == null) {
						...
						s.crossDomain = parts
								&& (parts.join(":") + (parts[3] ? ""
										: parts[1] === "http:" ? 80 : 443)) !== (ajaxLocParts
										.join(":") + (ajaxLocParts[3] ? ""
										: ajaxLocParts[1] === "http:" ? 80
												: 443));
					}
	...
	function Animation(elem, properties, options) {
		var result, index = 0, tweenerIndex = 0, length = animationPrefilters.length, deferred = jQuery
			...
			var currentTime = fxNow || createFxNow(), remaining = Math.max(0,
					animation.startTime + animation.duration - currentTime), percent = 1 - (remaining
					/ animation.duration || 0), index = 0, length = animation.tweens.length;
